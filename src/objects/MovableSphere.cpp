...
bool MovableSphere::IsHitByRay(const Ray& ray, double tMin, double tMax, HitRecord& hit) const
{
	Vector origin = GetCenterAtTime(ray.time);
...
Vector MovableSphere::GetCenterAtTime(double time) const
{
    return originStart + (originEnd - originStart) * ((time - timeStart) / (timeEnding - timeStart));
}
